...
int main(int argc, char *argv[])
{
    ...
    char data='A';
    int timerfd;
    struct itimerspec interval={{0,100000000L},{0,100000000L}};
    ...
    // set the TCP_USER_TIMEOUT socket option
    int tcp_timeout =1000000; //10 seconds before aborting a write()
    status = setsockopt(sockfd, SOL_TCP, TCP_USER_TIMEOUT, &tcp_timeout,sizeof(int));
    if (status < 0){
    ...
    while(running){
        ...
        pollReturn = poll(pollfds,nfds,4000); 
...
/*            // Debugging
            ...
            if(event(pollfds[0].revents, POLLIN)) {
                ...
                // set the TCP_USER_TIMEOUT socket option
                int tcp_timeout =1000000; //10 seconds before aborting a write()
                status = setsockopt(newsockfd, SOL_TCP, TCP_USER_TIMEOUT, &tcp_timeout,sizeof(tcp_timeout));
                if (status==-1){
                    error("TCP_USER_TIMEOUT");
                }
